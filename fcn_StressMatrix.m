function stressMat=fcn_StressMatrix(d,n,m,P,adjMat,fig)
stressMat = zeros(n, n);
% incidence matrix
H=zeros(m,n);
pointer=1;
...
if fig
    ...
    for i=1:n
        ...
        text(pi(1), pi(2), num2str(i),...
            'color', 'r', 'FontSize', 14, 'horizontalAlignment', 'center', 'FontName','Times New Roman');
    end
    % numbering edges
    pointer=1;
    for i=1:n
        for j=i+1:n
            if adjMat(i,j)~=0
                ...
                text((pi(1)+pj(1))/2, (pi(2)+pj(2))/2, num2str(pointer),...
                    'color', 'k', 'FontSize', 16, 'horizontalAlignment', 'center', 'FontName','Times New Roman');
                pointer=pointer+1;
            end
        end
    end
...
LMI=newlmi;
if num==0
    disp('info: B has 0 column; No self-stress')
    return;
elseif num==1
    disp('info: B has 1 column')
    x1=lmivar(1,[1 0]);
    % inequality: I<mu*(M1x1+M2x2+...)
    lmiterm([-LMI 1 1 x1],M(:,:,1),1); % right hand side
    LMIsys=getlmis;
    [~,x] = feasp(LMIsys,[0 0 1 0 0]);
elseif num==2
    disp('info: B has 2 columns')
    x1=lmivar(1,[1 0]);
    x2=lmivar(1,[1 0]);
    % inequality: I<mu*(M1x1+M2x2+...)
    lmiterm([-LMI 1 1 x1],M(:,:,1),1); % right hand side
    ...
    LMIsys=getlmis;
    [~,x] = feasp(LMIsys,[0 0 1 0 0]); % the third para is ||x||<1
elseif num>2
    disp('Error: B has more than 2 columns')
    return;
end
...
if max(eig(stressMat))>10^-3
    disp('Error: Stress Matrix is not PSD')
end
